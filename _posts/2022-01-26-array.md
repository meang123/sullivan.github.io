...
binary search 이용한 문제 많이 있다 lower bound, rotated array, find peak 문제도 binaray search로 접근 할수있다 

정렬이 되어있어야 사용할수있지만 일부분만 정렬 되어있어도 적용할 여지를 생각해야 한다 이 경우에는 mid 선택 해도 왼쪽으로 가야 할지 오른쪽으로 가야할지 정확히 판단이 안됀다 이때는 범위를 잘 정의 해야 한다 mid에 해당하는 값이 조건에 맞는 값인지 아닌지 판단 할수있어야 한다 예를 들어 조건을 만족하는 T와 F로 구분을 지을수있다(rotated array) 
...
3번 문제에서 binary search 로 확장하여 해결한 문제 있다 참고해라
...
정수 n, 2 * n, 3 * n을 이어 붙혔을때 1-9까지 digit 중복되면 안되고 0이 없는 거를 fascinating이라고 한다 fascinating이면 T 아니면 F 하는 문제 였다 
...
추가로 생각한 방법  조금더 직관적으로 생각해 보았다 while 조건 문에 idx2와 idx의 조건을 함께 고려 하는것은 적절하지 않았기 때문에 따로 종료문을 두기로 생각했고 그래서 무한 루프를 사용하기로 했다 idx가 nums.size()-1 위치 즉 배열 끝에 와있으면 종료 해야 한다 

그리고 추가로 고려 할것이 연속적으로 0이 아닌 숫자 나왔을때 이다 이때 idx가 idx2보다 같거나 더 커지면 idx2만 증가 시켜야 한다 

그리고 else if로 모두 한번식만 보기 때문에 각 조건문마다 종료 코드가 있어야 한다 
...
public:
    void moveZeroes(vector<int>& nums) 
    {
        ...
        while(true)
        {
            ...
            else if(nums[idx]==0&&nums[idx2]!=0)
            {
...
아래 코드도 위와 같은 맥락인데 조금 더 직관적으로 변경 하였다 
...
물론 위에 풀이가 시간 복잡도 면에서 더 좋다 o(n)이기 때문이다 아래에서 접근한 방식은 sliding window + binary search 이다 따라서 o(n log n) 복잡도 가진다 하지만 binary search space의 확장면에서 배운 부분이 있다 binary search로 풀때 찾고자 하는 문제를 minimal sub array length로 설정한다 mid 정하면 mid 가 length로 정의 되고 정의한 length만큼의 sliding window 진행한다 이때 target보다 크거나 같다면 더 작은 minimal size 찾기 위해 왼쪽으로 binary search 진행하고 반대로 다 작았다면 만족을 못한거니까 길이를 늘리는 방향 오른쪽에서 binary search 진행 한다 그래서 최소 길이가 0일수는 없으므로 left=1 right=size로 초기화 한다 
...
public:
    int minSubArrayLen(int target, vector<int>& nums)
    {
        ...
        while(left<=right)
        {
            ...
            {
                sum=accumulate(nums.begin()+i,nums.begin()+i+mid,0);
                if(sum>=target)
                {
                    flag=true;
                    len=mid;
...
위에 코드로 제출하면 시간 제한이 걸린다 sliding window로 sum하는 부분 accumulate에서 발생하는것 같다 그래서 누적합으로 먼저 계산하고 처리 하는 방향으로 최적화 해야 한다 



>  prifix sum [i] = num[0] ~ num[i-1] 까지의 합이다 
...
public:
    int minSubArrayLen(int target, vector<int>& nums)
    {
        ...
        int left=1,right = nums.size(),mid=0;
        bool flag =false;
		vector<long long> prefix(nums.size()+1,0LL); //0으로 누적합 배열 초기화 
        
        
        //nums[i..j-1] = prefix[j] - prefix[i]
...
원형 큐로 만들어서 k번 수행하면 될것으로 먼저 생각이 있었다. 그러나 벡터 두개있으면 해결 할것 같아서 생각대로 한 코드가 아래와 같다 
...
k가 size보다 큰 경우를 생각해야 한다 이때 사이클 도는 주기를 계산하면 된다 나머지 값을 새로운 k로 설정하고 start 계산하면 위와 같은 코드로 완성 가능하다 
...
같은 맥락인데 배열을 따로 만들지 않고 reverse를 이용하여 처리한 내용도 다른 사람들 풀이에서 확인할수있었다 

다른 벡터를 사용해야 할것으로 예상했는데 reverese이용한거를 배웠고 나머지 처리 계산하는것을 복잡하게 내가 한거에 비해 상당히 세련하게 처음부터 나머지식으로 계산하고 있다 
